			...
			<div class="row">
				<div class="col-md-12">
					<div class="search_pack">
						<form method="get" action="{{ route('site.home') }}">
						{{ csrf_field() }}
							<input type="hidden" name="hidden_key" value="{{ request()->query('hidden_key') }}">
							<input type="hidden" name="hidden_value" value="{{ request()->query('hidden_value') }}">
							
							<div class="col-md-8 col-lg-6">
							<div class="input-group">
							<input name="search" type="text" class="form-control" value="{{ request()->query('search')}}" placeholder="Enter your search option" aria-describedby="inputGroupPrepend2" required>
						...
						<div class="row" id="serchformktm">
							...
							{{--
							<div class="col-md-4 col-lg-3">
								<button class="btn advance_search">Topic</button>
							</div>
							--}}
							...
							{{--
							<div class="col-md-4 col-lg-3">
								<button class="btn advance_search">Author</button>
							</div>
							
							<div class="col-md-4 col-lg-3">
								<button class="btn advance_search">Title</button>
							</div>
							
							<div class="col-md-4 col-lg-3">
								<button class="btn advance_search">Motu / Island</button>
							</div>
							
							<div class="col-md-4 col-lg-3">
								<button class="btn advance_search">Iwi / Tribe</button>
							</div>
							
							<div class="col-md-4 col-lg-3">
								<button class="btn advance_search">Hapū / Village</button>
							</div>
							
							<div class="col-md-4 col-lg-3">
								<button class="btn advance_search">Person</button>
							</div>
							--}}
							...
							<div class="col-md-4 col-lg-3">
								<div class="select_box">
									<select name="sources" id="sources6" class="custom-select sources" placeholder="Language">
										<option value="1">Maori</option>
										<option value="2">Sāmoa</option>
								  </select>
								</div>
							</div>
...
@if(count($displayBooks) > 0)
<p style="text-align: center;font-size: 16px;font-weight: bold;margin-top: 15px;">Showing <span id="recordNumber">{{ count($displayBooks) }}</span> Records

@include('frontend.page.result')
...
@if(count($displayBooks) >= 2)
{{--
<div class="view_more">
	<a class="btn more_view" href="{{ $displayBooks->nextPageUrl() }}">View More</a>
</div>
--}}
@endif
...
$('.more_view').on('click', function(e){
	...
	newHref = '{{ route('site.home') }}?page='+pageNumber;
...
$('#clearSearch').on('click', function(){
	...
	if (typeof (history.pushState) != "undefined") {
        var obj = { Page: 'Search', Url: '{{ route('site.home') }}' };
